...
<div className="" id="container-package1">
  ...
  {loading ? (<div  style={{display:"flex",justifyContent:"center",alignItems:"center"}}><BeatLoader/></div>):(error?(toast.error("Error Loading Hotels")):(
    ...
    {data.map((item)=>(
        ...
        <img class="card-img-top" src={`/Image/${item.photos}`} alt="Cardcap"/>
        <div class="card-body">
          ...
          <button id={item._id} onClick={event=>addToFavorites(event)} style={{textDecoration:"none"}}><FontAwesomeIcon icon={faHeart} style={{cursor:'pointer'}}/></button>
          </div>
        </div>
      </div>
          </div>
